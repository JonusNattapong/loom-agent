...
describe("V0.4 eval harness",()=>{
  it("passes the V0.3 regression and V0.4 multi-agent scenarios",async()=>{
    ...
    expect(results.map(result=>result.name)).toEqual([
      ...
      "v0.6 planner fallback","v0.6 capability routing","v0.6 execution bounds","v0.6 crash planning recovery","v0.6 crash execution recovery","v0.6 crash review recovery","v0.7 durable job execution","v0.7 schedule occurrence dedupe",
    ]);
