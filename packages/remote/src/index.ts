...
export type RemoteJob = { jobId: string; payload: unknown; requiredCapabilities?: string[]; role?: string; priority?: number; allowedTools?: string[]; taskId?: string; rootAgentId?: string; workspaceId?: string };
...
export function validateEnvelope(value: unknown): ProtocolEnvelope {
  ...
  if(typeof v.type!=="string" || !((["register","heartbeat","dispatch","accepted","started","renew","ack","result","error","disconnect","cancel"] as string[]).includes(v.type))) throw new ProtocolValidationError("invalid_message_type","message type is invalid");
...
export class RemoteFabricController {
 ...
 async dispatchTool(input:{jobId:string;toolName:string;toolInput:Record<string,unknown>;allowedTools:string[];requiredCapabilities?:string[];taskId?:string;rootAgentId?:string;workspaceId?:string}){return this.dispatch({jobId:input.jobId,payload:{tool:{name:input.toolName,input:input.toolInput},allowedTools:input.allowedTools,taskId:input.taskId,rootAgentId:input.rootAgentId,workspaceId:input.workspaceId},allowedTools:input.allowedTools,requiredCapabilities:input.requiredCapabilities,taskId:input.taskId,rootAgentId:input.rootAgentId,workspaceId:input.workspaceId});}
 async dispatch(job:RemoteJob):Promise<RemoteDispatchResult>{const worker=new DeterministicRouter(this.registry).route(job,this.policy);if(!worker)throw new Error("no compatible online worker");const transport=this.transports.get(worker.workerId),connection=this.connections.get(worker.workerId);if(!transport||!connection)throw new Error("worker transport unavailable");const existing=this.state.getRemoteAssignment?.(job.jobId);if(existing&&existing.workerId===worker.workerId&&existing.status!=="expired"){const current=this.state.getRemoteLease?.(existing.leaseId);if(current&&current.status==="active")return {assignmentId:existing.id,workerId:worker.workerId,lease:{leaseId:current.id,workerId:current.workerId,fencingToken:current.fencingToken,expiresAt:new Date(current.expiresAt).toISOString()}};}const lease=this.leases.acquire(job.jobId,worker.workerId,30000);const assignment=this.state.assignRemote({workerId:worker.workerId,payload:job.payload,taskId:job.jobId,id:job.jobId});const durableLease=this.state.acquireRemoteLease(assignment.id,worker.workerId,30000);const effectiveLease={...lease,leaseId:durableLease.id,fencingToken:durableLease.fencingToken,expiresAt:new Date(durableLease.expiresAt).toISOString()};const sequence=this.state.nextProtocolSequence(connection.connectionId);const envelope=createEnvelope({type:"dispatch",id:assignment.id,sender:worker,connectionId:connection.connectionId,epoch:connection.epoch,sequence,payload:{assignmentId:assignment.id,executionId:job.jobId,requirements:job.requiredCapabilities??[],allowedTools:job.allowedTools??[],taskId:job.taskId,rootAgentId:job.rootAgentId,workspaceId:job.workspaceId,payload:job.payload,...(job.payload&&typeof job.payload==="object"?job.payload:{})},lease:effectiveLease});this.state.recordProtocolMessage({messageId:envelope.id,connectionId:connection.connectionId,direction:"outbound",sequence,envelope});await transport.send(envelope);this.state.setWorkerStatus(worker.workerId,"online");return {assignmentId:assignment.id,workerId:worker.workerId,lease:effectiveLease};}
 async receive(envelope:ProtocolEnvelope,bound?:{workerId:string;connectionId:string;instanceId:string;epoch:number}){if(!bound)throw new ProtocolValidationError("unbound_transport","inbound messages require a transport binding");const checked=validateEnvelope(envelope); envelope=checked; const worker=this.registry.get(bound.workerId);if(!worker||envelope.sender.workerId!==bound.workerId||envelope.sender.instanceId!==bound.instanceId||envelope.connectionId!==bound.connectionId||envelope.epoch!==bound.epoch)throw new ProtocolValidationError("unauthorized_connection","worker connection identity is not current");const connection=this.connections.get(bound.workerId);if(!connection||connection.connectionId!==bound.connectionId||connection.epoch!==bound.epoch)throw new ProtocolValidationError("stale_epoch","connection epoch is stale");const previous=this.receiveSequences.get(bound.connectionId)??0;if(envelope.sequence<=previous)return;if(envelope.sequence!==previous+1)throw new ProtocolValidationError("sequence_gap",`expected sequence ${previous+1}`);this.receiveSequences.set(bound.connectionId,envelope.sequence);if(envelope.type==="heartbeat"){this.registry.heartbeat(worker.identity.workerId,(envelope.payload as any)?.load);this.state.heartbeatWorker(worker.identity.workerId);return;}if(envelope.type==="ack"){const ack=Number((envelope.payload as any)?.sequence??envelope.sequence);if(!Number.isSafeInteger(ack)||ack<1)throw new ProtocolValidationError("invalid_ack","ack sequence is invalid");this.journals.get(worker.identity.workerId)?.acknowledge(Math.min(ack,this.journals.get(worker.identity.workerId)!.lastSequence));if(this.state.ackProtocolMessagesForWorker)this.state.ackProtocolMessagesForWorker(worker.identity.workerId,"outbound",ack);else this.state.ackProtocolMessage(connection.connectionId,"outbound",ack);return;}const executionId=String((envelope.payload as any)?.executionId??"");if(executionId){const payload:any=envelope.payload;const result=this.state.acceptRemoteEvent?this.state.acceptRemoteEvent({assignmentId:String(payload.assignmentId??executionId),workerId:worker.identity.workerId,leaseId:envelope.lease?.leaseId??"",fencingToken:envelope.lease?.fencingToken??-1,sequence:envelope.sequence,messageId:envelope.id,payload}):this.state.appendRemoteEvent({executionId,sequence:envelope.sequence,messageId:envelope.id,payload});if(!result.accepted)throw new Error("expired or stale remote execution");if(envelope.type==="renew"&&envelope.lease){this.state.renewRemoteLease(envelope.lease.leaseId,worker.identity.workerId,30000);return;}if(envelope.type==="accepted")this.state.updateRemoteAssignmentStatus(payload.assignmentId??executionId,"accepted");else if(envelope.type==="started")this.state.updateRemoteAssignmentStatus(payload.assignmentId??executionId,"running");else if(envelope.type==="result"){this.state.updateRemoteAssignmentStatus(payload.assignmentId??executionId,payload.status==="cancelled"?"cancelled":"completed");this.state.releaseRemoteLease(envelope.lease?.leaseId??"");}else if(envelope.type==="error"){this.state.updateRemoteAssignmentStatus(payload.assignmentId??executionId,"failed",typeof payload.error==="string"?payload.error:JSON.stringify(payload.error??"remote execution failed"));this.state.releaseRemoteLease(envelope.lease?.leaseId??"");}}}
 async cancel(assignmentId:string){const assignment=this.state.getRemoteAssignment(assignmentId);if(!assignment)throw new Error("remote assignment not found");const connection=this.connections.get(assignment.workerId),transport=this.transports.get(assignment.workerId),lease=this.state.getRemoteLease(assignment.leaseId);if(!connection||!transport||!lease)throw new Error("remote worker connection unavailable");this.state.updateRemoteAssignmentStatus(assignmentId,"cancelled","cancelled by controller");this.state.releaseRemoteLease(lease.id);const sequence=this.state.nextProtocolSequence(connection.connectionId);const envelope=createEnvelope({type:"cancel",id:`cancel_${assignmentId}`,sender:this.registry.get(assignment.workerId)!.identity,connectionId:connection.connectionId,epoch:connection.epoch,sequence,payload:{assignmentId,executionId:assignmentId,status:"cancelled"},lease:{leaseId:lease.id,workerId:lease.workerId,fencingToken:lease.fencingToken,expiresAt:new Date(lease.expiresAt).toISOString()}});this.state.recordProtocolMessage({messageId:envelope.id,connectionId:connection.connectionId,direction:"outbound",sequence,envelope});await transport.send(envelope);return assignment;}
 async reconnect(workerId:string,lastAck:number){const t=this.transports.get(workerId),journal=this.journals.get(workerId),connection=this.connections.get(workerId);if(!t||!journal||!connection)throw new Error("worker is not attached");const persisted=this.state.listUnackedProtocolMessagesForWorker?.(workerId,"outbound")??this.state.listUnackedProtocolMessages(connection.connectionId,"outbound");const envelopes=persisted.length?persisted.map((row:any)=>({...JSON.parse(row.envelope),connectionId:connection.connectionId,epoch:connection.epoch})):journal.replay(lastAck).map(envelope=>({...envelope,connectionId:connection.connectionId,epoch:connection.epoch}));for(const envelope of envelopes){await t.send(envelope);};this.state.heartbeatWorker(workerId,"online");}
}
