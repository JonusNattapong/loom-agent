...
export class RemoteControllerService{
 ...
 constructor(state:StateStore,options:WebSocketControllerOptions={}){this.transport=new WebSocketControllerTransport(options);this.fabric=new RemoteFabricController(state);this.transport.onWorker(worker=>this.fabric.attach(worker.identity,worker.transport,worker.capabilities,worker.load,worker.connectionId));}
