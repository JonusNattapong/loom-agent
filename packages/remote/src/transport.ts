...
export class WebSocketWorkerTransport implements WorkerTransport{
 ...
 async send(envelope:ProtocolEnvelope){if(!this.socket||this.socket.readyState!==WebSocket.OPEN)throw new Error("websocket is not open");this.socket.send(JSON.stringify(envelope));}
