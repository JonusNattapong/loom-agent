...
export class WebSocketControllerTransport{
 private server?:WebSocketServer; private listener?:ReturnType<typeof createServer>; private readonly sockets=new Map<string,SocketTransport>(); private workerHandler?: (worker:AuthenticatedWorker)=>void|{epoch?:number}|Promise<void|{epoch?:number}>;
 constructor(private readonly options:WebSocketControllerOptions={}){}
 onWorker(handler:(worker:AuthenticatedWorker)=>void|{epoch?:number}|Promise<void|{epoch?:number}>){this.workerHandler=handler;}
 async start(){if(this.server) return;const host=this.options.host??"127.0.0.1";if(!["127.0.0.1","::1","localhost"].includes(host)&&!this.options.tls)throw new Error("non-local remote worker exposure requires TLS");const path=this.options.path??"/v1/workers/connect";const maxPayload=this.options.maxMessageBytes??1024*1024;if(this.options.tls){const httpsServer=createServer({key:this.options.tls.key,cert:this.options.tls.cert});this.listener=httpsServer;this.server=new WebSocketServer({server:httpsServer,path,maxPayload});httpsServer.listen(this.options.port??4778,host);}else this.server=new WebSocketServer({host:host,port:this.options.port??4778,path,maxPayload});this.server.on("connection",socket=>this.handleConnection(socket));await new Promise<void>((resolve,reject)=>{this.server!.once("listening",()=>resolve());this.server!.once("error",reject);});}
 address(){return this.server?.address() as AddressInfo|undefined;}
 async stop(){if(!this.server)return;for(const socket of this.sockets.values())socket.close();await new Promise<void>(resolve=>this.server!.close(()=>resolve()));this.server=undefined;if(this.listener){const listener=this.listener;this.listener=undefined;await new Promise<void>((resolve,reject)=>listener.close(error=>error?reject(error):resolve()))}}
