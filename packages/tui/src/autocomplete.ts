...
export class CombinedAutocompleteProvider implements AutocompleteProvider {
	...
	async getSuggestions(
		...
		if (slashContext?.kind === "name") {
			const prefix = slashContext.prefix.startsWith("/") ? slashContext.prefix.slice(1) : slashContext.prefix;
			...
			if (!prefix) {
				filtered = commandItems.map((item) => ({
					value: item.name,
					label: item.label,
				...
				filtered = fuse.search(prefix).map((res) => ({
					value: res.item.name,
					label: res.item.label,
					...(res.item.description && { description: res.item.description }),
					...(res.item.argumentHint && { argumentHint: res.item.argumentHint }),
					...(res.item.sourceTag && { sourceTag: res.item.sourceTag }),
					...(res.item.takesArgument && { takesArgument: true }),
				}));
			}
